...
export const getMonthlyFinancialReport = async (req: Request, res: Response) => {
    try {
        ...
        const incomeResult = await query(
            'SELECT SUM("totalAmount") as "totalIncome", COUNT(id) as "orderCount" FROM "Order" WHERE status = \'Paid\' AND "createdAt" >= $1 AND "createdAt" <= $2',
            [startDate, endDate]
        );
        ...
        const staffResult = await query(
            'SELECT name, role, salary FROM "User" WHERE role IN (\'Cashier\', \'Waiter\', \'KitchenStaff\', \'Admin\') AND salary IS NOT NULL'
        );
        const totalSalaries = staffResult.rows.reduce((acc, user) => acc + Number(user.salary || 0), 0);

        // EXPENSE: Supplier Purchases
        const supplierResult = await query(
            'SELECT name, "itemType", "totalPurchaseAmount", "purchaseDate" FROM suppliers WHERE "purchaseDate" >= $1 AND "purchaseDate" <= $2',
            [startDate, endDate]
        );
