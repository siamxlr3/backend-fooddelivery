...
export const createCart = async (req: Request, res: Response) => {
    try {
        ...
        await query(
            'INSERT INTO cart ("userId", "foodId", quantity, "totalPrice", status, "updatedAt") VALUES ($1, $2, $3, $4, $5, NOW())',
            [userId, foodId, quantity, totalPrice, "Pending"]
        );
        if (client.isOpen) {
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        const result = await query(
            'UPDATE cart SET status = $1, "updatedAt" = NOW() WHERE id = $2 RETURNING *',
            [status, id]
        );
