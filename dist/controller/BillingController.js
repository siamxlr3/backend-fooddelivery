...
export const generateBill = async (req, res) => {
    try {
        ...
        if (manualTax !== undefined && manualDiscount !== undefined) {
            ...
            const settingsResult = await query('SELECT key, value FROM "Setting" WHERE key IN (\'tax_rate\', \'discount_rate\')');
        ...
        let bill;
        if (existingBill) {
            const updateResult = await query('UPDATE "Bill" SET subtotal = $1, tax = $2, discount = $3, "grandTotal" = $4 WHERE id = $5 RETURNING *', [subtotal, tax, discount, grandTotal, existingBill.id]);
            bill = updateResult.rows[0];
        }
        else {
            const createResult = await query('INSERT INTO "Bill" ("orderId", "invoiceNumber", subtotal, tax, discount, "grandTotal", "isPaid") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [orderId, invoiceNumber, subtotal, tax, discount, grandTotal, false]);
...
export const processPayment = async (req, res) => {
    ...
    try {
        ...
        const transactionResult = await client.query('INSERT INTO "Transaction" ("billId", amount, method, reference, "createdAt") VALUES ($1, $2, $3, $4, NOW()) RETURNING *', [billId, amount, method, reference]);
        const transaction = transactionResult.rows[0];
        // Update bill status if fully paid
        if (Number(amount) >= Number(bill.grandTotal)) {
            await client.query('UPDATE "Bill" SET "isPaid" = true, "paymentMethod" = $1 WHERE id = $2', [method, billId]);
            // Link to session if exists
            let currentSessionId = null;
            if (userID) {
                const sessionResult = await client.query('SELECT id FROM "Session" WHERE "userId" = $1 AND status = \'OPEN\' LIMIT 1', [userID]);
                if (sessionResult.rows[0])
                    currentSessionId = sessionResult.rows[0].id;
            }
            const updatedOrderResult = await client.query('UPDATE "Order" SET status = \'Paid\', "sessionId" = $1, "updatedAt" = NOW() WHERE id = $2 RETURNING *', [currentSessionId, bill.orderId]);
...
export const deleteBill = async (req, res) => {
    try {
        ...
        if (invoiceNumber) {
            const result = await query('DELETE FROM "Bill" WHERE "invoiceNumber" = $1 RETURNING *', [invoiceNumber]);
